...
export interface ZenmuxModelCapabilities {
  name: string;
  reasoning: boolean;
...
function readDiskCache(): Map<string, ZenmuxModelCapabilities> | undefined {
  try {
    ...
    const payload = JSON.parse(raw) as { models?: Record<string, unknown> } | unknown;
    if (!payload || typeof payload !== "object") return undefined;
    const models = (payload as { models?: Record<string, unknown> }).models;
...
interface ZenMuxPricingType {
  value: number;
  unit: string;
  currency: string;
}

interface ZenmuxModelApi {
  id: string;
  display_name: string;
...
function extractCost(p: ZenmuxModelApi["pricings"]): ZenmuxModelCapabilities["cost"] {
  const get = (a?: ZenMuxPricingType[]): number => a?.[0]?.value ?? 0;
  return {
    input: get(p.prompt),
...
function parseModel(model: ZenmuxModelApi): ZenmuxModelCapabilities {
...
async function doFetch(): Promise<void> {
  try {
    ...
    try {
      ...
      const data = (await response.json()) as { data?: ZenmuxModelApi[] };
